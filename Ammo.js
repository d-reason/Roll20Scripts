var Ammo = Ammo || (function() {
	...
	adjustAmmo = function (playerid,attr,amount,par) {
            ...
            case '!ammo':
				if(args.length > 1) {
					...
					if(chr) {
						...
						attr = findObjs({_characterid: chr.id, name: args[2]})[0];
					}
					amount=parseInt(args[3],10);
                    par=parseInt(args[4],10);
					if(attr) {
						adjustAmmo(msg.playerid,attr,amount,par);
					} else {
                        if(chr) {
                            sendMessage(
